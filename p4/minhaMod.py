...
class P4Switch(Switch):
    ...
    def start(self, controllers):
        cmd = (
            ...
            "--thrift-port {} "
            "{} &"
        ).format(self.thrift_port, self.json_path)
        self.cmd(cmd)

    def stop(self):
        self.cmd("pkill -f 'simple_switch .*{}' || true".format(self.json_path))
...
def create_topology(args):
    ...
    wifi_router = net.addHost("r1", cls=LinuxRouter, ip=wDefaultIP,
                              defaultRoute='via %s' % defaultExtIp)
    ...
    s1 = net.addSwitch(
        's1',
        cls=P4Switch,
        json_path='build/latencia_l2.json',
        thrift_port=9090
    )
    ...
    net.addLink(s2, router, intfName='r0-eth1',
                params2={'ip': '15.0.0.4/29'})

    net.addLink(s2, wifi_router, intfName='r1-eth1',
                params2={'ip': '15.0.0.5/29'})

    net.addLink(s3, wifi_router, intfName='r1-eth0',
